    ...
    public class unknown_properties : contexts.build_wrap
    {
        ...
        public void unknown_values_are_set()
        {
            PackageBuilderWithConfig.PropertiesValue
                .Check(val=>val.ElementAt(1).Check(_=>_.Key.ShouldBe("unknown")).ShouldBe("value", "value2"))
                .Check(val=>val.ElementAt(2).Check(_=>_.Key.ShouldBe("unknown2")).ShouldBe("value"));
        }
    }
}